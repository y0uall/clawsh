...
clawsh uses a custom binary protocol with end-to-end encryption over TCP or TLS. This document covers the wire format, cryptographic design, message types, and operational constraints. The protocol is implemented in `clawsh-proto`, the shared library used by both this handler and [clawsh-imp](https://github.com/y0uall/clawsh-imp).
...
The type byte allows the listener to route incoming connections from the first 6 bytes — agent sessions and raw shell sessions share the same port without ambiguity.
...
A heartbeat message is ~25–30 bytes. Without padding it would be trivially distinguishable from data traffic by size alone. With padding, the real length is only visible after decryption.

### Authentication

The agent's handshake includes `HMAC-SHA256(AgentInfo, PSK)`. The handler verifies this before completing the handshake. PSK derivation: `SHA256(passphrase)` → `[u8; 32]`. Prevents unauthorized agents from registering.
...
Encryption activates immediately after the handshake — the first heartbeat is already encrypted.
...
| `bincode` | Binary serialization of message payloads |
